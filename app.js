...
function saveSeekStep(sec) {
  ...
  state.seekStep = n;
  storage?.safeSet(SEEK_STEP_KEY, String(n));
}
...
try {
  const savedPageUrl = storage?.safeGet(PAGE_URL_KEY);
  if (savedPageUrl && pageUrl) pageUrl.value = savedPageUrl;
} catch { /* ignore */ }

function savePageUrl(url) {
  if (url) storage?.safeSet(PAGE_URL_KEY, url);
}

function loadWatchPositions() {
  const data = storage?.safeGetJson(WATCH_POS_KEY, {});
  return data && typeof data === 'object' ? data : {};
}

function writeWatchPositions(map) {
  storage?.safeSetJson(WATCH_POS_KEY, map);
}
...
function loadWatchSession() {
  const data = storage?.safeGetJson(WATCH_SESSION_KEY, null);
...
function saveWatchSession(partial = {}) {
  ...
  storage?.safeSetJson(WATCH_SESSION_KEY, next);
}

function clearWatchSession() {
  storage?.safeRemove(WATCH_SESSION_KEY);
}
...
function parseSubtitles(text, filename) {
  if (subtitleParser?.parseSubtitles) return subtitleParser.parseSubtitles(text, filename);
  return [];
}
